...
export default async function SignUp() {
  ...
  const signUpWithEmail = async (formData: FormData) => {
    ...
    redirect("/sign-in?verified=check_email")
  }
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="space-y-2 text-center text-xs text-muted-foreground">
            <p>The Free plan includes 5 posts per month on up to 2 platforms.</p>
            <p>
              Already have an account?{" "}
              <a href="/sign-in" className="text-brand underline">Sign in</a>
            </p>
          </div>
        </div>
      </main>
